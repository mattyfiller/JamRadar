...
function EventDetail({ id, events, onBack, onSave, savedIds, goingIds, onToggleGoing, followedOrgs, onToggleFollow, onOpenOrg }) {
  ...
  // Deep-linked event that's pending/archived/deleted — show a friendly
  // not-found screen instead of `return null` which leaves a blank page
  // with no way back. Mirrors the RiderProfile not-found pattern.
  if (!e) {
    return (
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg-base)' }}>
        <div style={{ padding: '14px 14px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid var(--line-soft)' }}>
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18 }}>Event not found</div>
        </div>
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: 'var(--fg-muted)', fontSize: 13, padding: 32, textAlign: 'center' }}>
          This event was archived, removed, or hasn't been approved yet.
        </div>
      </div>
    );
  }
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div className="mono" style={{ fontSize: 10, color: 'var(--fg-dim)' }}>
                {e.orgVerified ? 'Verified organizer' : 'Organizer'} · {e.going || 0} going
              </div>
            </div>
          </button>
          <button
...
// ───────────── NOTIFICATIONS ─────────────
function NotificationsScreen({ onBack, readIds = [], dynamicNotifs = [], onMarkAllRead, onOpenEvent }) {
  // Only include hardcoded NOTIFICATIONS seeds when the user has zero live
  // notifications. The seeds reference fake events ("Blue Mountain", "Sweet
  // Skis") — fine as empty-state filler, embarrassing alongside real pings.
  const all = dynamicNotifs.length > 0 ? dynamicNotifs : [...dynamicNotifs, ...NOTIFICATIONS];
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg-base)' }}>
      ...
      <div className="jr-scroll" style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 40px' }}>
        {all.map(n => {
          ...
          // Notification rows that reference an event become clickable —
          // tapping opens the event detail. Seed notifications (n.eventId
          // refers to a fake seed event) are non-clickable.
          const canOpen = n.eventId && typeof onOpenEvent === 'function';
          return (
            <div key={n.id}
              onClick={canOpen ? () => onOpenEvent(n.eventId) : undefined}
...
function CreateEventScreen({ prefs, onBack, onPublish, editing, findDuplicates }) {
  ...
  const buildEvent = () => {
    ...
    return {
      ...
      // Don't ship placeholder copy on real events. Empty values let the
      // event-detail screen omit those blocks entirely.
      location: location.trim() || editing?.location || null,
      desc: desc.trim() || editing?.desc || '',
